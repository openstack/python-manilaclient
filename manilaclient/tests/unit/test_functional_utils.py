...
"""
    ...
    @ddt.data(
        {
            'input': OUTPUT_LINES_SIMPLE,
            'valid_values': [['11', 'foo', 'BUILD'], ['21', 'bar', 'ERROR']],
        },
        {
    ...
    def test_multi_line_row_table_shifted_id_column(self):
        ...
        self.assertEqual(
            ['**', 'ID', 'Name', 'Status'], actual_result['headers']
        )
        self.assertEqual(valid_values, actual_result['values'])

    @ddt.data(
        {
            'input': OUTPUT_LINES_NESTED_TABLE,
            'valid_nested': {'headers': ['aa', 'bb'], 'values': []},
        },
        {
    ...
    @ddt.data(
        {
            'input_data': OUTPUT_LINES_DETAILS,
            'output_data': [
