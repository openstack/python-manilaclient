...
class SharesTest(utils.TestCase):
    ...
    @ddt.data(
        ...
        {'is_public': False, 'availability_zone': 'fake_azzzzz'},
    )
    @ddt.unpack
    def test_create_share_with_all_params_defined(
        self, is_public, availability_zone
    ):
        body = {
            'share': {
                'is_public': is_public,
        ...
        cs.shares.create(
            'nfs', 1, is_public=is_public, availability_zone=availability_zone
        )
        cs.assert_called('POST', '/shares', body)

    @ddt.data(
        {'mount_point_name': 'fake_mount_pt1'},
    ...
    def test_revert_to_snapshot_not_supported(self):
        ...
        self.assertRaises(
            client_exceptions.UnsupportedVersion,
    ...
    @ddt.data(
        ('id', 'b4991315-eb7d-43ec-979e-5715d4399827', True),
        ('id', 'b4991315-eb7d-43ec-979e-5715d4399827', False),
        ('path', 'fake_path', False),
        ('path', 'fake_path', True),
    )
    @ddt.unpack
    def test_list_shares_index_with_export_location(
        self, filter_type, value, detailed
    ):
        search_opts = {
        ...
        if detailed:
            cs.assert_called(
                'GET',
                (
                    '/shares/detail?export_location_'
                    + filter_type
                    + '='
                    + value
                    + '&is_public=True'
    ...
    def test_list_shares_index_with_is_soft_deleted(self, detailed):
        ...
        if detailed:
            cs.assert_called(
                'GET',
                ('/shares/detail?is_public=True' + '&is_soft_deleted=True'),
            )
        else:
            cs.assert_called(
    ...
    def test_list_shares_filter_by_share_type_alias(self):
        ...
        cs.assert_called(
            'GET', '/shares?is_public=True&sort_key=share_type_id'
        )
    ...
    @ddt.unpack
    def test_allow_access_to_share(
        self, access_to, access_type, action_name, microversion
    ):
    ...
    @ddt.data(
        {'access_to': 'localhost', 'access_type': 'ip', 'microversion': '2.0'},
        {'access_to': '127.0.0.*', 'access_type': 'ip', 'microversion': '2.0'},
        {
        ...
        {'access_to': '1', 'access_type': 'user', 'microversion': '2.0'},
        {'access_to': '1' * 3, 'access_type': 'user', 'microversion': '2.0'},
        {'access_to': '1' * 256, 'access_type': 'user', 'microversion': '2.0'},
        {'access_to': 'root+=', 'access_type': 'user', 'microversion': '2.0'},
        {'access_to': '', 'access_type': 'cert', 'microversion': '2.0'},
        {'access_to': ' ', 'access_type': 'cert', 'microversion': '2.0'},
        {'access_to': 'x' * 65, 'access_type': 'cert', 'microversion': '2.0'},
        {'access_to': 'alice', 'access_type': 'cephx', 'microversion': '2.0'},
        {'access_to': '', 'access_type': 'cephx', 'microversion': '2.13'},
        {
            'access_to': "bj\u00f6rn",
            'access_type': 'cephx',
            'microversion': '2.13',
    ...
    def test_set_metadata(self):
        ...
        cs.assert_called(
            'POST', '/shares/1234/metadata', {'metadata': {'k1': 'v2'}}
        )
    ...
    def test_metadata_update_all(self, share):
        ...
        cs.assert_called(
            'PUT', '/shares/1234/metadata', {'metadata': {'k1': 'v1'}}
        )
    ...
    def test_extend_share(self, microversion, action_name, force):
        ...
        with mock.patch.object(
            ...
            if not force:
                ...
                manager._action.assert_called_once_with(
                    action_name, share, {"new_size": size, "force": "true"}
                )
            self.assertEqual("fake", result)
