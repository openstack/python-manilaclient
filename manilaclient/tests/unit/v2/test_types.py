...
def get_valid_type_create_data_2_24():
    ...
    snapshot_none_combos = list(
        itertools.product(
            public, dhss, snapshot, create_from_snapshot, extra_specs
        )
    )
...
class TypesTest(utils.TestCase):
    ...
    def _add_standard_extra_specs_to_dict(
        self,
        extra_specs,
        create_from_snapshot=None,
        revert_to_snapshot=None,
    ...
    @ddt.unpack
    def test_create_2_24(
        self, is_public, dhss, snapshot, create_from_snapshot, extra_specs
    ):
        extra_specs = copy.copy(extra_specs)
    ...
    @ddt.data(
        (
            ...
            False,
            True,
            {'snapshot_support': True, 'replication_type': 'fake_repl_type'},
        ),
        (
            False,
            False,
            False,
            {'snapshot_support': False, 'replication_type': 'fake_repl_type'},
        ),
        (
            False,
            False,
    ...
    def test_set_key(self):
        ...
        t.set_keys({'k': 'v'})
        cs.assert_called(
            'POST', '/types/1/extra_specs', {'extra_specs': {'k': 'v'}}
        )
    ...
    def test_update(self, microversion):
        ...
        result = manager.update(share_type, name, is_public, description)
        expected_body['share_type']['description'] = description
        manager._update.assert_called_once_with(
            f"/types/{share_type}", expected_body, "share_type"
        )
        self.assertEqual("fake", result)
    ...
    def test_get_keys_from_resource_data(self):
        ...
        share_type = share_types.ShareType(
            mock.Mock(),
            {'extra_specs': valid_extra_specs, 'name': 'test'},
            loaded=True,
        )
    ...
    @ddt.data(
        {'prefer_resource_data': True, 'resource_extra_specs': {}},
        {
            'prefer_resource_data': False,
            'resource_extra_specs': {'fake': 'fake'},
        },
        {'prefer_resource_data': False, 'resource_extra_specs': {}},
    )
    @ddt.unpack
    def test_get_keys_from_api(
        self, prefer_resource_data, resource_extra_specs
    ):
        manager = mock.Mock()
        valid_extra_specs = {'test': 'test'}
