...
class QuotaSetsTest(utils.TestCase):
    ...
    def test_tenant_quotas_get(self, microversion):
        ...
        if version >= api_versions.APIVersion('2.25'):
            expected_url = f"{resource_path}/test/detail"
        else:
    ...
    def test_user_quotas_get(self, microversion):
        ...
        version = api_versions.APIVersion(microversion)
        if version >= api_versions.APIVersion('2.25'):
            expected_url = f"{resource_path}/test/detail?user_id=fake_user"
        else:
            expected_url = f"{resource_path}/test?user_id=fake_user"
    ...
    def test_share_type_quotas_get(self):
        ...
        expected_url = (
            f"{resource_path}/{tenant_id}/detail?share_type={share_type}"
        )
    ...
    def test_tenant_quotas_defaults(self, microversion):
        ...
        manager = self._get_manager(microversion)
        resource_path = self._get_resource_path(microversion)
        expected_url = f"{resource_path}/test/defaults"
        with mock.patch.object(
            manager, '_get', mock.Mock(return_value='fake_get')
        ):
    ...
    def test_update_share_type_quota(self, microversion):
        ...
        manager = self._get_manager(microversion)
        resource_path = self._get_resource_path(microversion)
        expected_url = f"{resource_path}/{tenant_id}?share_type={share_type}"
        expected_body = {
            'quota_set': {
                'tenant_id': tenant_id,
                'shares': 1,
    ...
    def test_update_share_type_quotas_for_share_networks(self):
        ...
        with mock.patch.object(
            manager, '_update', mock.Mock(return_value='fake_delete')
        ):
            self.assertRaises(
                ValueError,
                manager.update,
    ...
    def test_quotas_delete(self, microversion):
        ...
        expected_url = f"{resource_path}/test"
        with mock.patch.object(
            manager, '_delete', mock.Mock(return_value='fake_delete')
        ):
    ...
    def test_share_type_quotas_delete(self):
        ...
        manager = self._get_manager("2.39")
        resource_path = self._get_resource_path("2.39")
        expected_url = f"{resource_path}/test?share_type=fake_st"
        with mock.patch.object(
            manager, '_delete', mock.Mock(return_value='fake_delete')
        ):
    ...
    def test_share_type_quotas_using_old_microversion(self, operation):
        ...
        with mock.patch.object(
            manager, f'_{operation}', mock.Mock(return_value='fake_delete')
        ):
            self.assertRaises(
                TypeError,
            ...
            getattr(manager, f'_{operation}').assert_not_called()
