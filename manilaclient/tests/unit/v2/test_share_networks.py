...
class ShareNetworkTest(utils.TestCase):
    ...
    def test_list_not_detailed(self):
        with mock.patch.object(
            ...
            self.manager._list.assert_called_once_with(
                share_networks.RESOURCES_PATH, share_networks.RESOURCES_NAME
            )
    ...
    def test_list_with_filters(self):
        ...
        expected_path = (
            f"{share_networks.RESOURCES_PATH}/detail?all_tenants=1&status="
            "ERROR"
        )
        ...
        with mock.patch.object(
            ...
            self.manager._list.assert_called_once_with(
                expected_path, share_networks.RESOURCES_NAME
            )

    @ddt.data(
        *itertools.product(
            ["2.25", "2.26"], ['fake share nw', _FakeShareNetwork()]
        )
    )
    @ddt.unpack
    ...
    def test_update_with_exception(self):
        share_nw = 'fake share nw'
        self.assertRaises(
            exceptions.CommandError, self.manager.update, share_nw
        )
    ...
    def test_add_security_service(self):
        ...
        with mock.patch.object(self.manager, '_action', mock.Mock()):
            ...
            self.manager._action.assert_called_once_with(
                expected_path, share_nw, expected_body
            )
    ...
    def test_remove_security_service(self):
        ...
        with mock.patch.object(self.manager, '_create', mock.Mock()):
            ...
            self.manager._create.assert_called_once_with(
                expected_path, expected_body, share_networks.RESOURCE_NAME
            )
    ...
    def test_remove_security_service_from_share_nw_object(self):
        ...
        share_nw = self._FakeShareNetwork()
        expected_path = (
            share_networks.RESOURCE_PATH + '/action'
        ) % share_nw.id
        expected_body = {
            'remove_security_service': {
                'security_service_id': security_service.id,
