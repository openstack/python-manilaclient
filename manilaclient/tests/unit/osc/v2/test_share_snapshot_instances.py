...
class TestShareSnapshotInstanceList(TestShareSnapshotInstance):
    def setUp(self):
        ...
        self.cmd = osc_share_snapshot_instances.ListShareSnapshotInstance(
            self.app, None
        )

    def test_share_snapshot_instance_list(self):
        values = (
            osc_lib_utils.get_dict_properties(s._info, COLUMNS)
            for s in self.share_snapshot_instances
        )
    ...
    def test_share_snapshot_instance_list_detail(self):
        values = (
            osc_lib_utils.get_dict_properties(s._info, COLUMNS_DETAIL)
            for s in self.share_snapshot_instances
        )
...
class TestShareSnapshotInstanceSet(TestShareSnapshotInstance):
    def setUp(self):
        ...
        self.cmd = osc_share_snapshot_instances.SetShareSnapshotInstance(
            self.app, None
        )
