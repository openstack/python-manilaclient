...
class ShareInstanceDelete(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        if number_of_deletion_failures > 0:
            msg = _(
                "%(number_of_deletion_failures)s of "
...
class ShareInstanceList(command.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        data = (
            osc_utils.get_dict_properties(instance._info, columns)
...
class ShareInstanceSet(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            metavar="<status>",
            help=_(
                'Indicate which state to assign the instance. Options are: '
    ...
    def take_action(self, parsed_args):
        ...
        if not instance or not parsed_args.status:
            raise exceptions.CommandError(
                _("Nothing to set. Please define a '--status'.")
            )
