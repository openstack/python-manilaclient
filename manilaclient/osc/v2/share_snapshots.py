    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.wait:
            ...
            share_snapshot = utils.find_resource(
                share_client.share_snapshots, share_snapshot.id
            )
        ...
        for snapshot in parsed_args.snapshot:
            try:
                snapshot_obj = utils.find_resource(
                    share_client.share_snapshots, snapshot
                )
...
class SetShareSnapshot(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=_(
                "Assign a status to the snapshot (Admin only). "
        ...
        if parsed_args.status:
            try:
                share_client.share_snapshots.reset_state(
                    share_snapshot, parsed_args.status
                )
            except Exception as e:
                result += 1
                LOG.error(
...
class UnsetShareSnapshot(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            action='append',
            help=_(
                'Remove a property from snapshot '
...
class ListShareSnapshot(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            default=None,
            help=_(
                "Filter results matching a share snapshot name pattern. "
        ...
        parser.add_argument(
            ...
            default=None,
            help=_(
                "Filter results matching a share snapshot description "
        ...
        parser.add_argument(
            ...
            action=parseractions.KeyValueAction,
            help=_(
                'Filter snapshots having a given metadata key=value '
        ...
        parser.add_argument(
            ...
            default=False,
            help=_(
                "The total count of share snapshots before pagination is "
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.count:
            print(f"Total number of snapshots: {total_count}")
        return (
            columns,
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.wait:
            ...
            snapshot = utils.find_resource(
                share_client.share_snapshots, snapshot.id
            )
...
class AbandonShareSnapshot(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        if result > 0:
            total = len(parsed_args.snapshot)
            msg = _("%(result)s of %(total)s snapshots failed to abandon.") % {
                'result': result,
                'total': total,
            }
            raise exceptions.CommandError(msg)
...
class ShareSnapshotAccessDeny(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        for access_id in parsed_args.id:
            try:
                share_client.share_snapshots.deny(
                    snapshot=snapshot_obj, id=access_id
                )
            except Exception as e:
                result += 1
                LOG.error(
        ...
        if result > 0:
            ...
            msg = _(
                "Failed to delete access to a share snapshot for "
...
class ShareSnapshotShowExportLocation(command.ShowOne):
    ...
    def take_action(self, parsed_args):
        ...
        export_location = share_client.share_snapshot_export_locations.get(
            export_location=parsed_args.export_location, snapshot=snapshot_obj
        )
