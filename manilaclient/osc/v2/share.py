...
class CreateShare(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            'size', metavar="<size>", type=int, help=_('Share size in GiB.')
        )
        parser.add_argument(
            '--name',
            metavar="<name>",
        ...
        parser.add_argument(
            ...
            default=None,
            help=_(
                'Optional share group name or ID in which to create '
        ...
        parser.add_argument(
            ...
            default=None,
            help=_(
                'Optional custom export location. Available for '
                'microversion >= 2.84'
            ),
        )
        parser.add_argument(
            ...
            default=None,
            help=_(
                'Set encryption key reference i.e. UUID of the secret '
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.share_type:
            ...
            try:
                ...
                msg = (
                    "There is no default share type available. You must "
        ...
        if parsed_args.share_group:
            share_group = apiutils.find_resource(
        ...
        if parsed_args.snapshot_id:
            snapshot = apiutils.find_resource(
                share_client.share_snapshots, parsed_args.snapshot_id
            )
        ...
        if parsed_args.scheduler_hint:
            if share_client.api_version < api_versions.APIVersion('2.65'):
                ...
                if same_host_hint_shares:
                    ...
                    scheduler_hints['same_host'] = ','.join(
                        same_host_hint_shares
                    )
...
class DeleteShare(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            default=False,
            help=_(
                "Attempt forced removal of share(s), regardless of state "
                "(defaults to False)"
            ),
        )
        parser.add_argument(
    ...
    def take_action(self, parsed_args):
        ...
        if result > 0:
            ...
            msg = _("%(result)s of %(total)s shares failed to delete.") % {
                'result': result,
                'total': total,
            }
            raise exceptions.CommandError(msg)
...
class ListShare(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            help=_(
                'Filter shares by export location id or path. '
        ...
        parser.add_argument(
            ...
            action='store_true',
            help=_(
                'Get shares in recycle bin. If this parameter is set to '
        ...
        parser.add_argument(
            ...
            action=parseractions.KeyValueAction,
            help=_(
                'Filter shares having a given metadata key=value property '
        ...
        parser.add_argument(
            ...
            action=parseractions.KeyValueAction,
            help=_(
                'Filter shares with extra specs (key=value) of the share '
        ...
        parser.add_argument(
            ...
            default=None,
            help=_(
                "Filter results matching a share name pattern. "
        ...
        parser.add_argument(
            ...
            default=None,
            help=_(
                "Filter results matching a share description pattern."
                "Available only for microversion >= 2.36."
            ),
        )
        parser.add_argument(
            ...
            metavar="<encryption_key_ref>",
            help=_(
                'Filter shares by their encryption key ref. '
    ...
    def take_action(self, parsed_args):
        ...
        share_type_id = None
        if parsed_args.share_type:
            share_type_id = apiutils.find_resource(
                share_client.share_types, parsed_args.share_type
            ).id

        snapshot_id = None
        if parsed_args.snapshot:
            snapshot_id = apiutils.find_resource(
        ...
        elif getattr(parsed_args, 'name~') or getattr(
...
class ShowShare(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        parser = super().get_parser(prog_name)
        parser.add_argument(
            'share', metavar="<share>", help=_('Share to display (name or ID)')
        )
        return parser
...
class SetShare(command.Command):
    ...
    def get_parser(self, prog_name):
        parser = super().get_parser(prog_name)
        parser.add_argument(
            'share', metavar="<share>", help=_('Share to modify (name or ID)')
        )
        # 'metadata' --> 'properties'
        parser.add_argument(
            "--property",
        ...
        parser.add_argument(
            ...
            default=None,
            help=_(
                'Explicitly update the status of a share (Admin only). '
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.property:
            try:
                ...
                LOG.error(
                    _(
                        "Failed to set share properties "
        ...
        if kwargs:
            try:
                ...
                LOG.error(
                    _(
                        "Failed to update share display name, visibility "
...
class UnsetShare(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            action='append',
            help=_(
                'Remove a property from share '
                '(repeat option to remove multiple properties)'
            ),
        )
        parser.add_argument(
            '--name', action='store_true', help=_('Unset share name.')
        )
        parser.add_argument(
            '--description',
            action='store_true',
    ...
    def take_action(self, parsed_args):
        ...
        if kwargs:
            try:
                ...
                LOG.error(
                    _(
                        "Failed to unset share display name "
                        "or display description: %s"
                    ),
                    e,
                )
        ...
        if parsed_args.property:
            for key in parsed_args.property:
                try:
                    ...
                    LOG.error(
                        _("Failed to unset share property '%(key)s': %(e)s"),
                        {'key': key, 'e': e},
                    )
                    result += 1
...
class ResizeShare(command.Command):
    ...
    def get_parser(self, prog_name):
        parser = super().get_parser(prog_name)
        parser.add_argument(
            'share', metavar="<share>", help=_('Name or ID of share to resize')
        )
        parser.add_argument(
            'new_size',
            metavar="<new-size>",
        ...
        parser.add_argument(
            ...
            default=False,
            help=_(
                'Only applicable when increasing the size of the '
...
class AdoptShare(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            metavar="<share-server-id>",
            help=_(
                'Share server associated with share when using a share '
...
class AbandonShare(command.Command):
    ...
    def take_action(self, parsed_args):
        ...
        if result > 0:
            ...
            msg = _(
                "Failed to abandon %(result)s out of %(total)s shares."
...
class ShareExportLocationShow(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        parser = super().get_parser(prog_name)
        parser.add_argument(
            'share', metavar="<share>", help=_('Name or ID of share')
        )
        parser.add_argument(
            'export_location',
            metavar="<export-location>",
...
class ShareExportLocationList(command.Lister):
    ...
    def take_action(self, parsed_args):
        ...
        return (
            list_of_keys,
            (
                oscutils.get_item_properties(s, list_of_keys)
...
class ShareExportLocationSet(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            action=parseractions.KeyValueAction,
            help=_(
                "Set a property to this export location "
                "(repeat option to set multiple properties). "
    ...
    def take_action(self, parsed_args):
        ...
        if (
            ...
            and share_client.api_version < api_versions.APIVersion("2.87")
        ):
            raise exceptions.CommandError(
...
class ShareExportLocationUnset(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            action='append',
            help=_(
                "Remove a property from export location "
                "(repeat option to remove multiple properties). "
                "Available only for microversion >= 2.87."
            ),
        )
        ...
        if parsed_args.property:
            ...
            for key in parsed_args.property:
                try:
                    ...
                    result += 1
                    LOG.error(
                        "Failed to unset export location property "
                        "'%(key)s': %(e)s",
                        {'key': key, 'e': e},
                    )
            if result > 0:
...
class RevertShare(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            metavar="<snapshot>",
            help=_(
                'Name or ID of the snapshot to restore. The snapshot '
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.wait:
            if not oscutils.wait_for_status(
                ...
                raise exceptions.CommandError(
                    _("Share not available after revert attempt.")
                )
...
class ShareMigrationStart(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            metavar="<host>",
            help=_(
                "Destination host where share will be migrated to. Use the "
        ...
        parser.add_argument(
            ...
            default=False,
            help=_(
                "Enforces the use of the host-assisted migration approach, "
        ...
        parser.add_argument(
            ...
            required=True,
            help=_(
                "Enforces migration to be nondisruptive. If set to True, "
        ...
        parser.add_argument(
            ...
            default=None,
            help=_(
                "Specify the new share type for the share. Do not specify "
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.new_share_type:
            new_share_type_id = apiutils.find_resource(
                share_client.share_types, parsed_args.new_share_type
            ).id
        share = apiutils.find_resource(share_client.shares, parsed_args.share)
